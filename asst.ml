...
(* let rec get_post_cond z3ctx statement pt_bes pt_invs pt_assts pt_brs lems =  *)
(*   try  *)
(*     let fst_pt = fst statement in  *)
(*     let pre_cond0 = PtsMap.find fst_pt pt_bes in  *)
(*     let pt_bes_new =  *)
(*       if exists_asst fst_pt pt_assts  *)
(*       then *)
(* 				let asst = List.assoc fst_pt pt_assts in   *)
(* 				if not (dec_impl z3ctx pre_cond0 asst lems) *)
(* 				then let _ = raise_impl_err fst_pt pt_bes pt_assts in pt_bes *)
(* 				else PtsMap.add fst_pt asst pt_bes *)
(*       else *)
(* 				pt_bes *)
(*     in   *)
(*     let pre_cond pt =  *)
(*       let fm_res = fm_of_brs (PtsMap.find pt pt_brs) in  *)
(*       if fm_res = BeTrue  *)
(*       then PtsMap.find fst_pt pt_bes_new *)
(*       else BeAnd (fm_res, PtsMap.find fst_pt pt_bes_new) *)
(*     in  *)
(*     ( *)
(*       match statement with  *)
(*       | SSkip pt -> (pre_cond pt, pt_bes_new) *)
(*       | SAssg (pt, x, ae) ->  *)
(* 				 let nvar = Var (string_of_var x ^ "#" ^ string_of_pt pt) in *)
(* 				 let post =  *)
(* 					 BeEx (nvar,  *)
(* 								 (BeAnd  *)
(* 										(BeEq ((AeVar x), (subst_a ae x (AeVar nvar))),  *)
(* 										 subst_b (pre_cond pt) x (AeVar nvar)))) *)
(* 				 in (post, pt_bes_new) *)
(*       | SOt (pt, c, alst) -> (pre_cond pt, pt_bes_new) *)
(*       | SIn (pt, c, xlst) ->  *)
(* 				 let post =  *)
(* 					 List.fold_right (fun x fm -> BeEx (x, fm)) xlst (pre_cond pt) *)
(* 				 in (post, pt_bes_new) *)
(*       | SSeq (statement1, statement2) ->  *)
(* 				 let (post1, pt_bes1) =  *)
(* 					 get_post_cond  *)
(* 						 z3ctx statement1 pt_bes_new pt_invs pt_assts pt_brs lems in *)
(* 				 let pt_bes1' = PtsMap.add (fst statement2) post1 pt_bes1 in *)
(* 				 (\* let _ = print_endline (string_of_pt_be pt_bes1' ^ "\n") in *\) *)
(* 				 get_post_cond z3ctx statement2 pt_bes1' pt_invs pt_assts pt_brs lems *)
(*       | SIf (pt, be, statement1, statement2) -> *)
(* 				 let (post1, pt_bes1) =  *)
(* 					 let pre_cond1 = BeAnd (BeAnd (pre_cond pt, BeEt pt), be) in *)
(* 					 let mp1 = PtsMap.add (fst statement1) pre_cond1 pt_bes_new in  *)
(* 					 get_post_cond z3ctx statement1 mp1 pt_invs pt_assts pt_brs lems *)
(* 				 in *)
(* 				 let (post2, pt_bes2) =  *)
(* 					 let pre_cond2 = BeAnd (BeAnd (pre_cond pt, BeEf pt), BeNot be) in  *)
(* 					 let mp2 = PtsMap.add (fst statement2) pre_cond2 pt_bes_new in *)
(* 					 get_post_cond z3ctx statement2 mp2 pt_invs pt_assts pt_brs lems *)
(* 				 in *)
(* 				 (BeOr (post1, post2), use_snd pt_bes1 pt_bes2 (pts_of_stmt statement2)) *)
(*       | SWhile (pt, be, statement) ->  *)
(* 				 if not (exists_asst pt pt_invs)  *)
(* 				 then raise (Failure ("missing loop invariant at " ^ string_of_pt pt)) *)
(* 				 else *)
(* 					 let inv = List.assoc pt pt_invs in  *)
(* 					 if not (dec_impl z3ctx (pre_cond pt) inv lems)  *)
(* 					 then raise_inv_err1 pt inv *)
(* 					 else *)
(* 						 let pt_be_stmt =  *)
(* 							 PtsMap.add (fst statement) (BeAnd (inv, be)) pt_bes_new  *)
(* 						 in  *)
(* 						 let (post, pt_be') =  *)
(* 							 get_post_cond z3ctx statement pt_be_stmt pt_invs pt_assts pt_brs lems *)
(* 						 in *)
(* 						 if not (dec_impl z3ctx post inv lems)  *)
(* 						 then raise_inv_err2 pt inv *)
(* 						 else (BeAnd (inv, BeNot be), pt_be') *)
(*     ) *)
(*   with Not_found ->  *)
(*     raise (Failure "precondition not found") *)

(* let rec fvs_b = function *)
(*   | BeTrue | BeFalse | BeEt _ | BeEf _ | BeEbot _ -> VarSet.empty *)
(*   | BeEq (ae1, ae2) | BeNEq (ae1, ae2) -> VarSet.union (fvs_a ae1) (fvs_a ae2) *)
(*   | BeNot be' | BeEx (_, be') -> fvs_b be'  *)
...
(*   let neb = BeNot eb in *)
(*   BeOr (	 *)
(*       BeOr (BeAnd (BeAnd (et, nef), neb), BeAnd (BeAnd (net, ef), neb)),  *)
(*       BeAnd (BeAnd (net, nef), eb) *)
(*     ) *)

(* let post_cond_proc z3ctx (pr, statement) pt_invs pt_assts pt_brs =  *)
(*   let zterm x = BeEq (AeVar x, AeVal 0) in  *)
(*   let vars_of_stmt = vars_of_stmt statement in  *)
(*   let init_conj =  *)
(*     let min = VarSet.min_elt vars_of_stmt in  *)
(*     VarSet.fold  *)
(*       (fun x conj -> BeAnd (conj, zterm x))  *)
...
(*       (zterm min) *)
(*   in *)
(*   let init_pt_be = PtsMap.add (fst statement) init_conj (PtsMap.empty) in *)
(*   let brpts = brpts_of_stmt statement in  *)
(*   let lems = List.fold_left (fun acc brpt -> acc @ [mk_lem brpt]) [] brpts in *)
(*   let (post, pt_be) =  *)
(*     get_post_cond z3ctx statement init_pt_be pt_invs pt_assts pt_brs lems *)
(*   in *)
(*   PtsMap.add (Pt ("f_" ^ (string_of_prin pr))) post pt_be *)
	     
(* let post_cond_sys z3ctx (Sys (prstmts, _)) pt_invs pt_assts pt_brs =  *)
(*   List.fold_left  *)
(*     (fun acc prstmt ->  *)
(*      use_snd  *)
(*        acc  *)
(*        (post_cond_proc z3ctx prstmt pt_invs pt_assts pt_brs)  *)
(*        (pts_of_stmt (Pervasives.snd prstmt) @  *)
(* 	  [Pt ("f_" ^ (string_of_prin (Pervasives.fst prstmt)))]))  *)
(*     PtsMap.empty  *)
(*     prstmts *)
